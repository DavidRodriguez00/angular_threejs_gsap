...
import { SceneOneThree } from './scene-one.three';
...
export class SceneOneAnimations {
  ...
  init(): void {
    ...
    if (!this.three.sun || !this.three.planets.length) {
      console.warn('SceneOneAnimations: Three.js objects not initialized.');
      return;
    }
  ...
  private buildEntrance(): void {
    ...
    this.entranceTl
      ...
      .to(camera.position, {
        x: 0, y: 15, z: 35, // Valores alineados con CAM_OVERVIEW del SceneOneThree
        duration: 4,
        ease: 'power4.inOut',
        onUpdate: () => camera.lookAt(0, 0, 0),
        onStart: () => {
          // Habilitamos la lógica de rotación interna del SceneOneThree a mitad del vuelo
